...
<tr style="border-left: none;border-right: none; border-bottom: none;">
   ...
   <c:if test="${paging.startPage!=1 }">
      <a href="qna-out?nowPage=${paging.startPage-1 }&cntPerPage=${paging.cntPerPage}">◀</a>
      ...
      <c:forEach begin="${paging.startPage }" end="${paging.endPage}" var="p"> 
         <c:choose>
            ...
            <c:when test="${p != paging.nowPage }">
               <a href="qna-out?nowPage=${p}&cntPerPage=${paging.cntPerPage}">${p}</a>
            </c:when>   
         </c:choose>
      </c:forEach>
     
      <c:if test="${paging.endPage != paging.lastPage}">
      <a href="qna-out?nowPage=${paging.endPage+1}&cntPerPage=${paging.cntPerPage }">▶</a>
   </c:if>
